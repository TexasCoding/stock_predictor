...
class StockScreener:
    ...
    def filter_tickers(self) -> List[Dict[str, str]]:
        ...
        with console.status(
            ...
            for ticker in self.filtered_recommended_tickers:
                ...
                status.update(
                    f"[yellow]Analizing [bold]{ticker}[/bold] for future predictions..."
                )
                prediction = self._filter_tickers_predictions(ticker=ticker)
                if prediction:
                    status.update(
    ...
    def _filter_tickers_technicals(self) -> None:
        """
    ...
    def _filter_tickers_recommendations(self) -> None:
        """
    ...
    def _filter_tickers_predictions(self, ticker: str) -> Optional[Dict[str, str]]:
        """
